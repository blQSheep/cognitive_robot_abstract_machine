    ...
    with simulated_robot:
        sp = SequentialPlan(
            ...
            PlaceActionDescription(
                world.get_body_by_name("milk.stl"),
                PoseStamped.from_list(
                    [2.3, 2.5, 1.0], [0, 0, 0, 1], world.root
                ),
                Arms.LEFT,
            ),
        )
...
def test_transportAction(database, mutable_model_world):
    session = database
    world, robot_view, context = mutable_model_world
    action = SequentialPlan(
        Context.from_world(world),
        TransportActionDescription(
            world.get_body_by_name("milk.stl"),
            PoseStamped.from_list([2.9, 2.2, 0.95], [0, 0, 1, 0], world.root),
            Arms.LEFT,
        ),
    )
    with simulated_robot:
...
def test_parallel_plan(database, mutable_model_world):
    session = database
    world, robot_view, context = mutable_model_world
    plan = ParallelPlan(context, ParkArmsActionDescription(Arms.BOTH),
                        MoveTorsoActionDescription(TorsoState.HIGH))
